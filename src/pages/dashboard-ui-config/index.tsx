...
import {
  fetchDashboardUIConfigs,
  saveDashboardUIConfig,
} from "@/services/dashboardUIConfig";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function AppConfigs() {
  ...
  const [selectedItem, setSelectedItem] = useState<DashboardUiConfig | null>(
    null
  );
  ...
  const dialogContentClass = `${
    isFullWidth ? "w-[98vw] max-w-[98vw]" : "max-w-2xl"
  } max-h-[85vh] overflow-auto border rounded-lg shadow-lg`;
                          ...
                          <DropdownMenuContent>
                            ...
                            <DropdownMenuItem
                              onClick={() => {
                                setSelectedItem(item);
                                setEditedJson(
                                  JSON.stringify(item.uiJson, null, 2)
                                );
                                setIsFullWidth(true);
                                setIsEditing(true);
                                setIsDuplicating(false);
            ...
            <div className="flex justify-end gap-2 mt-4">
              <Button
                ...
                onClick={() => {
                  ...
                  setNewConfigName("");
                  if (selectedItem)
                    setEditedJson(JSON.stringify(selectedItem.uiJson, null, 2));
                  setJsonError(null);
                }}
              >
                Cancel
              ...
              <Button
                ...
                disabled={
                  !!jsonError ||
                  saving ||
                  (isDuplicating && !newConfigName.trim())
                }
              >
