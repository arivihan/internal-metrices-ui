...
interface BulkUploadVideoDialogProps {
  open: boolean;
...
export function BulkUploadVideoDialog({
  open,
  onOpenChange,
  onSuccess,
}: BulkUploadVideoDialogProps) {
  ...
  const loadBatchOptions = async () => {
    setLoading(true);
    try {
      console.log("[BulkUploadVideoDialog] Loading batch options...");
      const batchesRes = await fetchAllBatchesForVideos({ activeFlag: true });
      console.log("[BulkUploadVideoDialog] Batch options loaded:", batchesRes);
      ...
      console.error("[BulkUploadVideoDialog] Failed to load batch options:", error);
      toast.error(
  ...
  const validateFiles = (files: File[]): File[] => {
    ...
    const maxSize = 10 * 1024 * 1024; // 10MB per file for Excel
    const validTypes = [
    ...
    files.forEach((file) => {
      ...
      if (!hasValidType && !hasValidExtension) {
        toast.error(
          `${file.name}: Invalid format. Please upload Excel files (.xlsx, .xls).`
        );
        return;
      }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      if (validFiles.length > 0) {
        ...
        toast.success(`${validFiles.length} Excel file(s) added`);
      }
    }
  };
  ...
  const validateForm = (): boolean => {
    if (formData.files.length === 0) {
      toast.error("Please select at least one Excel file to upload");
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Bulk upload response received:", response);
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
            ...
            <div
              ...
              className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed px-6 py-8 transition-all ${
                isDragging
                  ...
                  : "border-muted-foreground/25 hover:border-green-300 hover:bg-green-50/50"
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Supported formats: .xlsx, .xls (max 10MB each)
              </p>
            </div>
            ...
            {formData.files.length > 0 && (
              <div className="space-y-2">
                <Label className="text-sm text-muted-foreground">
                  {formData.files.length} Excel file(s) selected
                </Label>
          ...
          <div className="rounded-lg border bg-blue-50 p-4">
            ...
            <ul className="text-xs text-blue-700 space-y-1">
              <li>• Column A: Video URL (required)</li>
              <li>• Column B: Thumbnail URL (optional)</li>
              <li>• Column C: Video Code (optional, auto-generated if empty)</li>
              <li>• Column D: Video Type (CHAMPIONS_OF_ARIVIHAN, TESTIMONIAL, etc.)</li>
              <li>• Column E: Orientation (PORTRAIT or LANDSCAPE)</li>
              <li>• Column F: Display Context (HOMESCREEN, COURSE_PAGE, etc.)</li>
            </ul>
            <Button
              variant="link"
              size="sm"
              className="text-blue-700 px-0 mt-2"
              onClick={() => {
                // Could implement template download here
          ...
          <div className="space-y-4">
            ...
            >
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <strong>Files Processed:</strong> {formData.files.length}
                </div>
                <div>
                  <strong>Batch:</strong>{" "}
                ...
                {uploadPreview?.uploadedVideos !== undefined && (
                  <div>
                    <strong>Videos Added:</strong> {uploadPreview.uploadedVideos}
                  </div>
                )}
                {uploadPreview?.failedVideos !== undefined && uploadPreview.failedVideos > 0 && (
                  <div>
                    <strong className="text-red-600">Failed:</strong>{" "}
                    <span className="text-red-600">{uploadPreview.failedVideos}</span>
                  </div>
                )}
                <div>
            ...
            {/* Error Details */}
            {uploadPreview?.errorDetails && uploadPreview.errorDetails.length > 0 && (
              <div className="border rounded-lg border-red-200 bg-red-50">
                <div className="p-3 border-b border-red-200">
                  <span className="font-medium text-sm text-red-800">
                    Error Details ({uploadPreview.errorDetails.length})
                  </span>
                </div>
                <ScrollArea className="h-32">
                  <div className="p-3 space-y-1">
                    {uploadPreview.errorDetails.map((error: string, idx: number) => (
            ...
            <div className="border rounded-lg">
              ...
              {showFullDetails && (
                <div className="border-t p-4 max-h-60 overflow-y-auto">
                  <pre className="text-xs p-3 rounded whitespace-pre-wrap break-words bg-muted">
